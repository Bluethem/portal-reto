...
import { generateLevel, colorName } from "./generator.ts";
import { phraseHint } from "../ia/groq.ts";
import type { GroqEnv } from "../ia/groq.ts";
...
function sendHint(j: Judge, target: string, text: string, remaining: number): void {
  void j.room.send({ content: { type: "hint", text, remaining, target } });
}

function currentDirectorId(j: Judge): string | null {
...
async function buildHint(j: Judge): Promise<string> {
  ...
  const ai = await phraseHint(process.env as GroqEnv, color, j.level);
  return ai ?? fallback;
}

async function handleHintRequest(j: Judge, senderId: string): Promise<void> {
