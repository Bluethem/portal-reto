...
- **Nivel ≥ 5**: corte malo → 50% de chance de añadir un global (`lockCut` 1.5 s o
  `blind` 3 s). Y **periódico**: cada ~25 s un global aleatorio (`blind` 3 s /
  `lockCut` 1.5 s).
...
- El board lee `RoomState.effects` en `subscribeState` y aplica: overlays
  (freeze/blind) y deshabilitado de hits (freeze/lockCut). Los efectos se
  auto-limplan con el siguiente publish al expirar.
- `theme.css`: `.effect-layer`, `.effect-overlay`, `.effect-freeze` y
  `.effect-blind`.
...
- Smoke 4 jugadores: niveles 1-3 sin efectos; nivel ≥ 4 corte malo congela al
  culpable; nivel ≥ 5 niebla/bloqueo periódicos; limpieza al expirar y al subir
  de nivel.
